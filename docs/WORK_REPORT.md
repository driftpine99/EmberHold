...
- **Lokale Commits:** drei — `ee42993` (Gates A+B), `4741870` (Gates C/D/E +
  neue Verträge), `chore(d043-gate-e)` + `docs(d043)` (toten CSS/Docs,
  direkt danach). **Nicht gepusht.**
...
| B – Ruhe + kein Ring | erfüllt | Stroke-Ring entfernt; Aura auf `lr*0.55` verkleinert (kantenlos). Kachel: Grundton `#38445f`, Mikrosterne/Leitungen/Knoten entfernt, 2 unregelmäßige Fugen statt 128px-Raster, Nebel reduziert; Sternebene 44→12 schwach; Landmarks nur `h>0.88` (Planet `h>0.965`) mit Alpha .42/.55; Warden-Seals nur `zelle>0.78`, Props nur `zelle<0.10` (je 1 statt 4). D-017/Simulation/Culling unberührt | `combatArtV4.ringWeg/kulisseRuhig`; `combatUiV2` präzisiert: misst verhaltensbasiert NUR den Boden-Pass (Deko-Ebenen temporär abgehängt, Kamera versetzt auf x=137) → 7 vs 9 Kacheln, deckBisZumRand true; renderCostContract: weiterhin 2 Gradienten/Frame, 0 shadowBlur, 0 Canvas/Frame |
...
| Avatar | Lichtkreis-Gradient bis lr + 8px-Strokering | kantenlose Aura auf 0.55·lr, kein Ring |
...
1. Klingenfokus bezieht sich auf das erste ANGEBOT (Reroll verbraucht die
   Garantie) — wie D-042 dokumentiert, unverändert.
...
2. Falls „erster Kartenzug“ beim Klingenfokus doch ab der ersten WAHL (statt
   des ersten Angebots) gemeint ist, bitte als Einzeiler-Followup definieren.
